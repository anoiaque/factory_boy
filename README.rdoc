...
After each test everything is unstubbed. That means, if you have a case where a particular(complex) query is executed
but not right stubbed with factory boy you can test using fixtures in databases(with factory girl or just model.create ..), skipping factory boy process.
...
Assign fixtures to association in definition of plant :
  ...
  Plant.define :address do |adress|
    adress.number = 12
