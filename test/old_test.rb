...
def test_define_with_association_has_many
  ...
  adress = Adress.find
  assert_equal adress, user.addresses.first
end
...
def test_create_plant_with_hash_for_values
  adress = Plant(:adress)
  user = Plant(:user, :name => "Marie", :addresses => [adress])
...
def test_define_with_class_option
  Plant.define :marie, :class => User do |peter|
    peter.name = "Marie"
    peter.addresses = [Adress.new(:street => "Rue de Brest")]
  end
  marie = Plant(:marie)
...
def test_define_with_dependent_attribute
  Plant.define :user do |user|
    user.name = "Marie"
    user.addresses = [Adress.new(:street => "Rue de #{user.name}")]
  end
...
def assert_find_is_unstubbed_for_each_class
  [Address, Customer, Profile, User].each {|klass| assert_equal "original_find", klass.find }
end